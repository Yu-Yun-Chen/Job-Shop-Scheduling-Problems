...
def fitFunc_diff(x):            # 適應度函數
    ...
    for m in m_keys:
        if m_count[m]>=maximum:
            maximum = m_count[m];
        if m_count[m]<=minimum:
            minimum = m_count[m];
    #print("max: ", maximum, ",  min: ", minimum)
    diff = maximum - minimum
    
    return -diff           # 因為是最小化問題
...
pop = initPop()             # 初始化 pop
pop_fit = evaluatePop(pop)  # 算 pop 的 fit => fitness 是difference
...
print("fitFunc: ", -fitFunc_makespan(best_chromosome))
